	...
	for (uint16_t flow_id = 0; flow_id < parameters->IO_Flow_Definitions.size(); flow_id++)
	{
		...
		case Flow_Type::SYNTHETIC:
		{
			...
			io_flow = new Host_Components::IO_Flow_Synthetic(this->ID() + ".IO_Flow.Synth.No_" + std::to_string(flow_id), flow_id,
				...
				parameters->Enable_ResponseTime_Logging, parameters->ResponseTime_Logging_Period_Length, parameters->Input_file_path + ".IO_Flow.No_" + std::to_string(flow_id) + ".log", (unsigned int)parameters->IO_Flow_Definitions.size());
...
void Host_System::decide_whether_to_stop_engine()
{
	bool stop = true;
