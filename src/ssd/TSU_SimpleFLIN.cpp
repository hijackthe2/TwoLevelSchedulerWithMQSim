...
{
	TSU_SimpleFLIN::TSU_SimpleFLIN(const sim_object_id_type& id, FTL* ftl, NVM_PHY_ONFI_NVDDR2* NVMController, const unsigned int Channel_no,
		...
		: TSU_Base(id, ftl, NVMController, Flash_Scheduling_Type::SIMPLE_FLIN, Channel_no, chip_no_per_channel, die_no_per_chip, plane_no_per_die,
	...
	size_t TSU_SimpleFLIN::GCTRQueueSize(flash_channel_ID_type channel_id, flash_chip_ID_type chip_id)
	{
	...
	void TSU_SimpleFLIN::Validate_simulation_config()
	{
	}
	void TSU_SimpleFLIN::Execute_simulator_event(MQSimEngine::Sim_Event*)
	{
	...
	void TSU_SimpleFLIN::Prepare_for_transaction_submit()
	{
	...
	void TSU_SimpleFLIN::Submit_transaction(NVM_Transaction_Flash* transaction)
	{
		transaction_receive_slots.push_back(transaction);
	}
	void TSU_SimpleFLIN::Schedule0()
	{
		opened_scheduling_reqs--;
	...
	void TSU_SimpleFLIN::reorder_for_fairness(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator start,
	...
	void TSU_SimpleFLIN::estimate_alone_waiting_time(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator position)
	{
	...
	double TSU_SimpleFLIN::fairness_based_on_average_slowdown(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator start,
		stream_id_type& flow_with_max_average_slowdown)
	{
	...
	void TSU_SimpleFLIN::move_forward(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator TRnew_pos,
	...
	bool TSU_SimpleFLIN::service_read_transaction(NVM::FlashMemory::Flash_Chip* chip)
	{
	...
	bool TSU_SimpleFLIN::service_write_transaction(NVM::FlashMemory::Flash_Chip* chip)
	{
		auto& info = transaction_waiting_dispatch_slots[chip->ChannelID][chip->ChipID].front();
		std::list<Flash_Transaction_Queue*> queue_list;
	...
	bool TSU_SimpleFLIN::service_erase_transaction(NVM::FlashMemory::Flash_Chip* chip)
	{
	...
	void TSU_SimpleFLIN::service_transaction(NVM::FlashMemory::Flash_Chip* chip)
	{
		if (_NVMController->GetChipStatus(chip) != ChipStatus::IDLE)
			return;
		if (!MappingReadTRQueue[chip->ChannelID][chip->ChipID].empty())
	...
	void TSU_SimpleFLIN::estimate_proportional_wait(NVM_Transaction_Flash_RD* read_slot, NVM_Transaction_Flash_WR* write_slot,
	...
	NVM_Transaction_Flash_RD* TSU_SimpleFLIN::get_read_slot(flash_channel_ID_type channel_id, flash_chip_ID_type chip_id)
	{
	...
	NVM_Transaction_Flash_WR* TSU_SimpleFLIN::get_write_slot(flash_channel_ID_type channel_id, flash_chip_ID_type chip_id)
	{
		NVM_Transaction_Flash* slot = NULL;
	...
	void TSU_SimpleFLIN::estimate_alone_time(NVM_Transaction_Flash* transaction, Flash_Transaction_Queue* queue)
	{
	...
	bool TSU_SimpleFLIN::is_dominated_by_one_stream(Flash_Transaction_Queue* queue)
	{
	...
	void TSU_SimpleFLIN::move_alone_time(NVM_Transaction_Flash* forward_transaction, NVM_Transaction_Flash* backward_transaction)
	{
	...
	void TSU_SimpleFLIN::adjust_alone_time(stream_id_type dispatched_stream_id, sim_time_type adjust_time, Transaction_Type type,
	...
	void TSU_SimpleFLIN::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter& xmlwriter)
	{
