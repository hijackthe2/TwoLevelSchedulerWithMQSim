#include "TSU_FuzzyFLIN.h"

namespace SSD_Components
{
	TSU_FuzzyFLIN::TSU_FuzzyFLIN(const sim_object_id_type& id, FTL* ftl, NVM_PHY_ONFI_NVDDR2* NVMController, const unsigned int Channel_no,
		...
		: TSU_Base(id, ftl, NVMController, Flash_Scheduling_Type::FUZZY_FLIN, Channel_no, chip_no_per_channel, die_no_per_chip, plane_no_per_die,
	...
	size_t TSU_FuzzyFLIN::GCTRQueueSize(flash_channel_ID_type channel_id, flash_chip_ID_type chip_id)
	{
	...
	size_t TSU_FuzzyFLIN::UserTRQueueSize(stream_id_type gc_stream_id, flash_channel_ID_type channel_id, flash_chip_ID_type chip_id)
	{
		size_t sz = 0;
	...
	size_t TSU_FuzzyFLIN::UserTRQueueSize(flash_channel_ID_type channel_id, flash_chip_ID_type chip_id)
	{
		return UserReadTRQueue[channel_id][chip_id].size() + UserWriteTRQueue[channel_id][chip_id].size();
	}

	void TSU_FuzzyFLIN::Start_simulation()
	{
		Simulator->Register_sim_event(flow_classification_epoch, this, 0, 0);
	}
	void TSU_FuzzyFLIN::Validate_simulation_config()
	{
	}
	void TSU_FuzzyFLIN::Execute_simulator_event(MQSimEngine::Sim_Event*)
	{
		...
		for (unsigned int channel_id = 0; channel_id < channel_count; channel_id++)
		{
			for (unsigned int chip_id = 0; chip_id < chip_no_per_channel; chip_id++)
			{
				...
				{
					if (is_low_intensity(flow_activity_info[channel_id][chip_id][stream_id].Serviced_read_requests_recent, alpha_read_for_epoch))
					...
					if (is_low_intensity(flow_activity_info[channel_id][chip_id][stream_id].Serviced_write_requests_recent, alpha_write_for_epoch))
	...
	void TSU_FuzzyFLIN::handle_transaction_serviced_signal(NVM_Transaction_Flash* transaction)
	{
	}

	void TSU_FuzzyFLIN::Prepare_for_transaction_submit()
	{
	...
	void TSU_FuzzyFLIN::Submit_transaction(NVM_Transaction_Flash* transaction)
	{
		transaction_receive_slots.push_back(transaction);
	}
	void TSU_FuzzyFLIN::Schedule0()
	{
		opened_scheduling_reqs--;
	...
	bool TSU_FuzzyFLIN::is_low_intensity(unsigned int requests_recent, unsigned int alpha_for_epoch)
	{
		if (requests_recent <= alpha_for_epoch) return true;
		double probability = 1.0 - (double)alpha_for_epoch / requests_recent;
		double prob = (double)(rand() % 1000) / 1000;
		return prob > probability;
	}

	void TSU_FuzzyFLIN::reorder_for_fairness(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator start,
	...
	void TSU_FuzzyFLIN::estimate_alone_waiting_time(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator position)
	{
	...
	double TSU_FuzzyFLIN::fairness_based_on_average_slowdown(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator start,
		stream_id_type& flow_with_max_average_slowdown)
	{
	...
	void TSU_FuzzyFLIN::move_forward(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator TRnew_pos,
	...
	bool TSU_FuzzyFLIN::service_read_transaction(NVM::FlashMemory::Flash_Chip* chip)
	{
	...
	bool TSU_FuzzyFLIN::service_write_transaction(NVM::FlashMemory::Flash_Chip* chip)
	{
		auto& info = transaction_waiting_dispatch_slots[chip->ChannelID][chip->ChipID].front();
		std::list<Flash_Transaction_Queue*> queue_list;
		...
		flash_die_ID_type die_id = queue_list.front()->front()->Address.DieID;
		flash_plane_ID_type page_id = queue_list.front()->front()->Address.PageID;
		bool has_dispatched = false;
		for (unsigned int i = 0; i < die_no_per_chip; i++)
		{
			transaction_dispatch_slots.clear();
	...
	bool TSU_FuzzyFLIN::service_erase_transaction(NVM::FlashMemory::Flash_Chip* chip)
	{
	...
	void TSU_FuzzyFLIN::service_transaction(NVM::FlashMemory::Flash_Chip* chip)
	{
		if (_NVMController->GetChipStatus(chip) != ChipStatus::IDLE)
			return;
		if (!MappingReadTRQueue[chip->ChannelID][chip->ChipID].empty())
	...
	void TSU_FuzzyFLIN::estimate_proportional_wait(NVM_Transaction_Flash_RD* read_slot, NVM_Transaction_Flash_WR* write_slot,
	...
	NVM_Transaction_Flash_RD* TSU_FuzzyFLIN::get_read_slot(flash_channel_ID_type channel_id, flash_chip_ID_type chip_id)
	{
	...
	NVM_Transaction_Flash_WR* TSU_FuzzyFLIN::get_write_slot(flash_channel_ID_type channel_id, flash_chip_ID_type chip_id)
	{
		NVM_Transaction_Flash* slot = NULL;
	...
	void TSU_FuzzyFLIN::estimate_alone_time(NVM_Transaction_Flash* transaction, Flash_Transaction_Queue* queue)
	{
	...
	bool TSU_FuzzyFLIN::is_dominated_by_one_stream(Flash_Transaction_Queue* queue)
	{
	...
	void TSU_FuzzyFLIN::move_alone_time(NVM_Transaction_Flash* forward_transaction, NVM_Transaction_Flash* backward_transaction)
	{
	...
	void TSU_FuzzyFLIN::adjust_alone_time(stream_id_type dispatched_stream_id, sim_time_type adjust_time, Transaction_Type type,
	...
	void TSU_FuzzyFLIN::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter& xmlwriter)
	{
