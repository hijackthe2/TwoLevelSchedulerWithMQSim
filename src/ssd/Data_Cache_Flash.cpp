...
namespace SSD_Components
{
	...
	Data_Cache_Slot_Type Data_Cache_Flash::Get_slot(const stream_id_type stream_id, const LPA_type lpn)
	{
		LPA_type key = LPN_TO_UNIQUE_KEY(stream_id, lpn);
		auto it = slots.find(key);
		assert(it != slots.end());//若key存在slots中
		if (lru_list.begin()->first != key)//如果要查找的数据没有在LRU队列的第一个位置
	...
	{
		assert(slots.size() > 0);
		slots.erase(lru_list.back().first);//。back（）返回最后一个元素
		Data_Cache_Slot_Type evicted_item = *lru_list.back().second;
		delete lru_list.back().second;
		lru_list.pop_back();//删除最后一个元素
		return evicted_item;
	}
		...
		const data_timestamp_type timestamp, const page_status_type state_bitmap_of_write_sectors)
	{
		...
		lru_list.push_front(std::pair<LPA_type, Data_Cache_Slot_Type*>(key, cache_slot));//在头部添加元素
		cache_slot->lru_list_ptr = lru_list.begin();//LRU指针指向于第一个元素
		slots[key] = cache_slot;
		/*if (slots.size() < 32768)
		{
			if (slots.size() % 100 == 0)
