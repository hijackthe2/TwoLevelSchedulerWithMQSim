...
namespace SSD_Components
{
			...
			EraseSuspensionEnabled, ProgramSuspensionEnabled)
	{
		...
		read_unit = new TSU_SLUnit(channel_count * chip_no_per_channel * 4, stream_count);
		write_unit = new TSU_SLUnit(channel_count * chip_no_per_channel * 4, stream_count);
	}
	...
	void TSU_SLFIFO::enqueue_transaction_for_speed_limit_type_tsu()
	{
		if (stream_count > 1)
		{
			read_unit->discharging(UserReadTRQueue);
			write_unit->discharging(UserWriteTRQueue);
		}
	}
}
