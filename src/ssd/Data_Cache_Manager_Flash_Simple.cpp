...
namespace SSD_Components
{
	...
	//对Data_Cache_Manager_Flash_Simple类中的Setup_triggers()函数进行定义
		...
		if (user_request->Type == UserRequestType::READ)
		{
			...
			case Caching_Mode::WRITE_CACHE:
			{
				...
				while (it != user_request->Transaction_list.end())
				{
					...
					if (data_cache->Exists(tr->Stream_id, tr->LPA))//读的逻辑地址存在cache中
					{
				...
				if (user_request->Transaction_list.size() > 0)//处理写回的数据？
					static_cast<FTL*>(nvm_firmware)->Address_Mapping_Unit->Translate_lpa_to_ppa_and_dispatch(user_request->Transaction_list);
	...
	void Data_Cache_Manager_Flash_Simple::Execute_simulator_event(MQSimEngine::Sim_Event* ev)
	{
		...
		dram_execution_queue[transfer_inf->Stream_id].pop();//删除栈顶元素
		if (dram_execution_queue[transfer_inf->Stream_id].size() > 0)//如果还有栈顶的元素存在
		{
			Memory_Transfer_Info* new_transfer_info = dram_execution_queue[transfer_inf->Stream_id].front();//返回起始元素的引用
