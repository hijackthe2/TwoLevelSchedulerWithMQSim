#include "TSU_SLFFLIN.h"

namespace SSD_Components
{
	TSU_SLFFLIN::TSU_SLFFLIN(const sim_object_id_type& id, FTL* ftl, NVM_PHY_ONFI_NVDDR2* NVMController, const unsigned int Channel_no,
		...
		: TSU_Base(id, ftl, NVMController, Flash_Scheduling_Type::SLF_FLIN, Channel_no, chip_no_per_channel, die_no_per_chip, plane_no_per_die,
	...
	size_t TSU_SLFFLIN::GCTRQueueSize(flash_channel_ID_type channel_id, flash_chip_ID_type chip_id)
	{
	...
	void TSU_SLFFLIN::Start_simulation()
	{
		Simulator->Register_sim_event(flow_classification_epoch, this, 0, 0);
	}
	void TSU_SLFFLIN::Validate_simulation_config()
	{
	}
	void TSU_SLFFLIN::Execute_simulator_event(MQSimEngine::Sim_Event*)
	{
	...
	void TSU_SLFFLIN::Submit_transaction(NVM_Transaction_Flash* transaction)
	{
		transaction_receive_slots.push_back(transaction);
	}
	void TSU_SLFFLIN::Schedule0()
	{
		opened_scheduling_reqs--;
	...
	void TSU_SLFFLIN::first_stage(Flash_Transaction_Queue* queue, NVM_Transaction_Flash* transaction,
	...
	void TSU_SLFFLIN::reorder_for_fairness(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator start,
	...
	void TSU_SLFFLIN::estimate_alone_waiting_time(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator position)
	{
	...
	double TSU_SLFFLIN::fairness_based_on_average_slowdown(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator start,
		stream_id_type& flow_with_max_average_slowdown)
	{
	...
	void TSU_SLFFLIN::move_forward(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator TRnew_pos,
	...
	bool TSU_SLFFLIN::service_read_transaction(NVM::FlashMemory::Flash_Chip* chip)
	{
	...
	bool TSU_SLFFLIN::service_write_transaction(NVM::FlashMemory::Flash_Chip* chip)
	{
		auto& info = transaction_waiting_dispatch_slots[chip->ChannelID][chip->ChipID].front();
		std::list<Flash_Transaction_Queue*> queue_list;
	...
	bool TSU_SLFFLIN::service_erase_transaction(NVM::FlashMemory::Flash_Chip* chip)
	{
	...
	NVM_Transaction_Flash_RD* TSU_SLFFLIN::get_read_slot(flash_channel_ID_type channel_id, flash_chip_ID_type chip_id)
	{
	...
	NVM_Transaction_Flash_WR* TSU_SLFFLIN::get_write_slot(flash_channel_ID_type channel_id, flash_chip_ID_type chip_id)
	{
		NVM_Transaction_Flash* slot = NULL;
	...
	bool TSU_SLFFLIN::is_dominated_by_one_stream(Flash_Transaction_Queue* queue)
	{
	...
	void TSU_SLFFLIN::move_alone_time(NVM_Transaction_Flash* forward_transaction, NVM_Transaction_Flash* backward_transaction)
	{
	...
	void TSU_SLFFLIN::adjust_alone_time(stream_id_type dispatched_stream_id, sim_time_type adjust_time, Transaction_Type type,
	...
	void TSU_SLFFLIN::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter& xmlwriter)
	{
